...
#810 (next revalidate/tags/cache options now forwarded by all CRUD methods — createResource, createFileResource, updateResource and deleteResource pass options.next to fetch; jest-tested),
#801 (authenticated JSON:API index: getIndex accepts withAuth (default false, public index) for sites that protect /jsonapi — jest-tested),
#553 (get* helper functions now carry @deprecated JSDoc tags pointing to the DrupalClient class methods — IDEs will strike them through),
#761 (NextFetchRequestConfig: already supported — every data method accepts JsonApiWithNextFetchOptions and forwards next/cache; resolved before the sweep),
#800 (translate-path without anonymous access: the client already supports withAuth on translatePath; the permission itself is a Drupal-side question),
#435 (paragraph in paragraph: reporter solved it with the jsonapi_default extension — resolution recorded in the issue),
#471 + #477 (umami login: resolved by reporters — client_id is the consumer UUID, keys placement; the guide and FAQ entries cover both),
#529 (CONTRIBUTING docs: CONTRIBUTING.md exists and TESTING.md documents the local suites and the 86 network-failure baseline),
#534 (replace Yarn v1: folded into the dev-dependencies modernization project — lockfile regeneration is the prerequisite),
#607 (patching the module from PRs: patchLevel -p3 note; CONTRIBUTING.md now exists as the entry point),
#691 (docs for 2.0.0: upstream PR #787 updated the docs — resolved),
#767 (one Drupal, many frontends: covered by site resolvers and the filter-by-site guide),
#773 (preview errors on Next 14 app router: needs-repro; the draft mode guide documents the app router flow),
#786 (preview while editing: deferred UI enhancement, same family as #217/#446),
#794 (reverse relationships: Corresponding Entity References (cer) is the Drupal-side answer, as noted in the issue),
#798 (per-request custom headers: deferred API enhancement; workaround is one client per context, as the reporter did),
#819 (replace the renderer decoration with a preview route: deferred architectural refactor, tied to #217),
#825 (access token debugging: needs-info, no follow-up on the requested details),
#829 (improve Typedoc docs: deferred docs tooling enhancement),
#834 (repeatable local environment: addressed on the fork by the deterministic e2e pipeline — install-drupal.sh + seeded site + Cypress, documented in TESTING.md),
#871 (middleware for image routing: deferred enhancement — next.config rewrites or middleware pattern to document),
#885 (DrupalCon BoF announcement: community event, out of scope),
#891 (close the Drupal.org issue queue: maintainer action on d.o, not actionable here).
