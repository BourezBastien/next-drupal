...
- ✅ **Migration Playwright LIVRÉE (session 29, audit intégral)** : suite Playwright miroir des specs Cypress (`test/e2e/playwright/` + config), `package.json` dédié dans `test/e2e` (indépendant du yarn.lock racine). **Playwright 13/13 ET Cypress 13/13 validés dans la même session** sur le même pipeline. Les environnements sans téléchargement navigateur utilisent `PLAYWRIGHT_CHANNEL=msedge`. La bascule CI Playwright attend le chantier modernisation (lock).
