...
### Redirecting to the created resource

The created resource is returned deserialized, so for entities with a path
field (like nodes) you can read the alias of the new resource directly from
the response — no extra API call needed.
...
// article.path.alias is available when the bundle has a path field,
// e.g. "/articles/my-new-article".
if (article.path?.alias) {
  redirect(article.path.alias)
}
```
