...
Not every entity type is reachable by path. Media items, for instance,
return 404 unless **Allow users to access media items at /media/{id}** is
enabled in the Drupal media settings at `/admin/config/media/media-settings`.
Check the entity type's path settings and permissions when a path resolves
to 404.
...
Fetch a paginated collection with its JSON:API metadata — deserialized results
plus `meta` (with the result count) and `links` (with `next` and `prev`).

```ts
const { results, meta, links } = await drupal.getResourceCollection(
  "node--article",
  {
    withMeta: true,
    params: {
