...
The OAuth token response does not include the Drupal user details. To expose
them, add the user data to the JWT claims on the Drupal side using
 ...
 * Implements hook_simple_oauth_private_claims_alter().
 */
function my_module_simple_oauth_private_claims_alter(array &$private_claims, AccessTokenEntity $access_token_entity) {
  $user = User::load($access_token_entity->getUserIdentifier());
...
Then decode the access token in the Next.js `session` callback to expose the
claims to the client, as done in the
[Umami demo](https://github.com/chapter-three/next-drupal/tree/main/examples/example-umami):
...
session.user.email = decoded.mail
