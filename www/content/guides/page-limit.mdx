...
The file lives at `sites/default/services.yml` in your Drupal project. If it does not exist yet — a fresh Drupal installation only ships `sites/default/default.services.yml` — copy that template and remove the leading `default.` prefix:

```sh
cp sites/default/default.services.yml sites/default/services.yml
```

Then add the parameter to the copied file:

```yaml
parameters:
  # Set your max limit here.
  next_jsonapi.size_max: 100
```

Finally clear the caches — `drush cr` — for the parameter to take effect.
