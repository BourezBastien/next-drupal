...
**Warning: keep `withAuth` out of public page requests.** If you fetch
pages with `withAuth: true` (or set `withAuth` globally on the client),
every page render runs as this scope and **unpublished content becomes
visible to all visitors**. Reserve authenticated requests for preview
and draft mode, and fetch public pages unauthenticated — optionally
filtering with `filter[status]=1` so unpublished content never renders.
